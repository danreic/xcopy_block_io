...
$(NVME_LIB_TMP):
	...
	@echo "Creating minimal nvme library (TCP only) from $(NVME_LIB_SRC)..."
	@mkdir -p /tmp/spdk_nvme_extract
	@cd /tmp/spdk_nvme_extract && ar x $(NVME_LIB_SRC)
	@cd /tmp/spdk_nvme_extract && rm -f nvme_cuse.o nvme_vfio_user.o nvme_rdma.o nvme_fc.o 2>/dev/null || true
